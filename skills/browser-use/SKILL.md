...
4.  **Read the page yourself**: `extract` is DENIED — it answers the question by spending a
    separate LLM call on the page, and you are already the model reading it. Use `state`,
    `get text`, `get html`, or `screenshot` and draw your own conclusion. `close`, `run`,
    `init` and `python` are denied too; `browser-head` owns the session lifecycle.
5.  **Troubleshooting**:
